...
<Section title="Overview">
    Scroll is a ZK-Rollup that targets bytecode-level compatibility with the Ethereum Virtual Machine. The protocol features a centralised sequencer operated by the protocol team. The zkEVM circuits are based on Ethereum Foundation's Privacy and Scaling Group research work on zkEVM.
